...
def get_live_results():
    ...
    try:
        results_file = "live_results/live_bot_history.json"
        if os.path.exists(results_file):
            with open(results_file, 'r') as f:
                data = json.load(f)
            return data
        else:
            return {"error": "No live results found", "metadata": {"total_cycles": 0, "total_signals": 0, "total_trades": 0}}
    except Exception as e:
        return {"error": f"Failed to load live results: {str(e)}", "metadata": {"total_cycles": 0, "total_signals": 0, "total_trades": 0}}
