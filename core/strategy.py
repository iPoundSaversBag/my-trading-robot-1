...
class MLMarketRegimeDetector:
    """Advanced ML-based market regime detection with ensemble methods"""
    ...
    def __init__(self, params: dict):
        ...
        try:
            ...
            self.ensemble_classifier = GradientBoostingClassifier(
                n_estimators=50,
                learning_rate=0.1,
                random_state=42
            )
            ...
            self.logger.warning("Scikit-learn not available - using fallback regime detection")
        ...
        # WALK FORWARD OPTIMIZATION: Pre-training support with configuration
        optimization_settings = params.get('optimization_settings', {})
        self.enable_pretraining = params.get('USE_ML_REGIME_DETECTION', False) and optimization_settings.get('ml_pretraining_enabled', True)
        self.pretraining_window_size = optimization_settings.get('ml_pretraining_window_size', 500)
        self.pretraining_step_size = optimization_settings.get('ml_pretraining_step_size', 100)
        self.max_pretraining_samples = optimization_settings.get('ml_pretraining_max_samples', 50)
        self.min_training_features = optimization_settings.get('ml_min_training_features', 10)

    def _create_synthetic_regime_features(self, target_regime: str, df: pd.DataFrame) -> list:
        """Create synthetic features for missing regimes to ensure ML training diversity"""
        try:
            synthetic_features = []
            base_features = None
            
            # Extract base features from a sample window
            if len(df) >= 200:
                sample_window = df.iloc[-200:]
                base_features = self.extract_regime_features(sample_window)
            
            if not base_features:
                return []
            
            # Create 2-3 synthetic samples per missing regime
            for i in range(3):
                features = base_features.copy()
                
                # Modify features to match target regime characteristics
                if target_regime == 'trending_bull':
                    features['sma_trend_short'] = 0.015 + i * 0.005  # Strong positive trend
                    features['sma_trend_medium'] = 0.010 + i * 0.003
                    features['momentum'] = 0.020 + i * 0.005
                    features['rsi'] = 0.65 + i * 0.05
                    
                elif target_regime == 'trending_bear':
                    features['sma_trend_short'] = -0.015 - i * 0.005  # Strong negative trend
                    features['sma_trend_medium'] = -0.010 - i * 0.003
                    features['momentum'] = -0.020 - i * 0.005
                    features['rsi'] = 0.35 - i * 0.05
                    
                elif target_regime == 'ranging':
                    features['sma_trend_short'] = 0.002 * (i - 1)  # Minimal trend
                    features['sma_trend_medium'] = 0.001 * (i - 1)
                    features['momentum'] = 0.005 * (i - 1)
                    features['volatility_ratio'] = 0.8 + i * 0.1
                    
                elif target_regime == 'high_volatility':
                    features['volatility_1d'] = 0.04 + i * 0.01  # High volatility
                    features['volatility_ratio'] = 1.5 + i * 0.2
                    features['atr_ratio'] = 0.03 + i * 0.005
                    
                elif target_regime == 'low_volatility':
                    features['volatility_1d'] = 0.01 - i * 0.002  # Low volatility
                    features['volatility_ratio'] = 0.7 - i * 0.05
                    features['atr_ratio'] = 0.015 - i * 0.002
                    
                elif target_regime == 'breakout_bullish':
                    features['volume_ratio'] = 2.0 + i * 0.3  # High volume
                    features['momentum'] = 0.025 + i * 0.005  # Strong momentum
                    features['price_position_20d'] = 0.8 + i * 0.05
                    features['volatility_ratio'] = 1.2 + i * 0.1
                    
                elif target_regime == 'breakout_bearish':
                    features['volume_ratio'] = 2.0 + i * 0.3  # High volume
                    features['momentum'] = -0.025 - i * 0.005  # Strong negative momentum
                    features['price_position_20d'] = 0.2 - i * 0.05
                    features['volatility_ratio'] = 1.2 + i * 0.1
                    
                elif target_regime == 'accumulation':
                    features['volume_ratio'] = 1.3 + i * 0.1  # Building volume
                    features['momentum'] = 0.005 * (i - 1)  # Minimal momentum
                    features['price_position_20d'] = 0.3 + i * 0.1
                    features['rsi'] = 0.35 + i * 0.05  # Oversold conditions
                    
                elif target_regime == 'distribution':
                    features['volume_ratio'] = 1.1 - i * 0.05  # Decreasing volume
                    features['momentum'] = -0.005 * i  # Slight negative momentum
                    features['price_position_20d'] = 0.8 + i * 0.05  # High price position
                    features['rsi'] = 0.65 + i * 0.05  # Overbought conditions
                
                # Ensure all features are within reasonable bounds
                for key, value in features.items():
                    if key == 'rsi':
                        features[key] = max(0.0, min(1.0, value))
                    elif key in ['price_position_20d', 'price_position']:
                        features[key] = max(0.0, min(1.0, value))
                    elif key in ['volatility_1d', 'volatility_5d', 'atr_ratio']:
                        features[key] = max(0.001, min(0.1, value))
                    elif key == 'volume_ratio':
                        features[key] = max(0.1, min(5.0, value))
                
                synthetic_features.append(features)
            
            self.logger.info(f"Created {len(synthetic_features)} synthetic samples for {target_regime}")
            return synthetic_features
            
        except Exception as e:
            self.logger.error(f"Error creating synthetic features for {target_regime}: {e}")
            return []
    
    def pretrain_on_historical_data(self, df: pd.DataFrame) -> bool:
        """
        ENHANCED: Pre-train ML model on historical data with forced regime diversity
        
        This method extracts features from multiple sliding windows and ensures
        we get samples from all 9 market regimes for robust ML training.
        """
        if not self.sklearn_available or not self.enable_pretraining:
            return False
            
        try:
            self.logger.info(f"Pre-training ML model on {len(df)} historical data points")
            
            # ENHANCED: Use diverse sampling strategy
            training_features = []
            regime_counts = {}
            target_per_regime = max(3, self.max_pretraining_samples // 9)  # Aim for balanced samples
            
            # Strategy 1: Sample from different market periods
            window_size = min(self.pretraining_window_size, len(df) // 4)
            step_size = max(self.pretraining_step_size, window_size // 10)  # Smaller steps for more diversity
            
            # Strategy 2: Sample from different volatility periods
            # Calculate volatility for each period to ensure diverse conditions
            close = df['close']
            returns = close.pct_change().dropna()
            rolling_vol = returns.rolling(50).std()
            
            # Find high, medium, and low volatility periods
            vol_quartiles = rolling_vol.quantile([0.25, 0.5, 0.75]).values
            low_vol_periods = rolling_vol[rolling_vol <= vol_quartiles[0]].index
            med_vol_periods = rolling_vol[(rolling_vol > vol_quartiles[0]) & (rolling_vol <= vol_quartiles[2])].index
            high_vol_periods = rolling_vol[rolling_vol > vol_quartiles[2]].index
            
            self.logger.info(f"Volatility periods: Low={len(low_vol_periods)}, Med={len(med_vol_periods)}, High={len(high_vol_periods)}")
            
            # Sample from each volatility regime
            for vol_period_name, vol_indices in [("low", low_vol_periods), ("med", med_vol_periods), ("high", high_vol_periods)]:
                if len(vol_indices) == 0:
                    continue
                    
                # Sample multiple windows from this volatility period
                samples_from_period = 0
                vol_indices_list = vol_indices.tolist()  # Convert to list of positions
                
                for i, idx in enumerate(vol_indices_list[::max(1, len(vol_indices_list) // 20)]):  # Take every Nth index
                    # Find the integer position of this timestamp in the dataframe
                    try:
                        ...
                        if end_idx - start_idx >= window_size:
                            ...
                            if features:
                                # Check what regime this would be classified as
                                ...
                                # Add sample if we need more of this regime
                                if regime_counts.get(regime_name, 0) < target_per_regime:
                                    training_features.append(features)
                                    regime_counts[regime_name] = regime_counts.get(regime_name, 0) + 1
                                    samples_from_period += 1
                                    
                                    if samples_from_period >= 10:  # Limit per volatility period
                                        break
                    except (KeyError, ValueError):
                        continue  # Skip if index not found
                
                self.logger.info(f"Collected {samples_from_period} samples from {vol_period_name} volatility period")
            
            # Strategy 3: Force missing regimes by adjusting thresholds
            missing_regimes = []
            all_regimes = ['trending_bull', 'trending_bear', 'ranging', 'high_volatility', 'low_volatility', 
                          'breakout_bullish', 'breakout_bearish', 'accumulation', 'distribution']
            
            for regime in all_regimes:
                if regime_counts.get(regime, 0) == 0:
                    missing_regimes.append(regime)
            
            if missing_regimes:
                self.logger.info(f"Missing regimes: {missing_regimes}, attempting forced sampling...")
                
                # Create synthetic samples for missing regimes by modifying feature thresholds
                for regime in missing_regimes:
                    synthetic_features = self._create_synthetic_regime_features(regime, df)
                    if synthetic_features:
                        training_features.extend(synthetic_features)
                        regime_counts[regime] = regime_counts.get(regime, 0) + len(synthetic_features)
            
            self.logger.info(f"Final regime distribution: {regime_counts}")
            unique_regimes = len([r for r in regime_counts.values() if r > 0])
            
            if len(training_features) >= 10 and unique_regimes >= 5:  # Need at least 5 regimes
                # Populate feature history
                self.feature_history = training_features
                
                # Force model training
                self._train_initial_model()
                
                self.logger.info(f"Enhanced pre-training completed: {len(training_features)} samples, {unique_regimes} regimes, trained={self.model_trained}")
                return self.model_trained
            else:
                self.logger.warning(f"Insufficient pre-training diversity: {len(training_features)} samples, {unique_regimes} regimes")
                return False
                
        except Exception as e:
            self.logger.error(f"Error in enhanced ML pre-training: {e}")
            import traceback
    ...
    def extract_regime_features(self, df: pd.DataFrame) -> dict:
        ...
        try:
            ...
            vol_1d = returns.rolling(20).std().iloc[-1] if len(returns) > 20 else 0.02
            vol_5d = returns.rolling(100).std().iloc[-1] if len(returns) > 100 else 0.02
            
            # Handle NaN values from rolling calculations
            features['volatility_1d'] = vol_1d if not pd.isna(vol_1d) else 0.02
            features['volatility_5d'] = vol_5d if not pd.isna(vol_5d) else 0.02
            features['volatility_ratio'] = features['volatility_1d'] / features['volatility_5d'] if features['volatility_5d'] > 0 else 1.0
            ...
            if len(close) >= 200:
                trend_short = (close.iloc[-1] - sma_20.iloc[-1]) / sma_20.iloc[-1]
                trend_medium = (sma_20.iloc[-1] - sma_50.iloc[-1]) / sma_50.iloc[-1]
                trend_long = (sma_50.iloc[-1] - sma_200.iloc[-1]) / sma_200.iloc[-1]
                
                # Handle NaN values
                features['sma_trend_short'] = trend_short if not pd.isna(trend_short) else 0.0
                features['sma_trend_medium'] = trend_medium if not pd.isna(trend_medium) else 0.0
                features['sma_trend_long'] = trend_long if not pd.isna(trend_long) else 0.0
            else:
                # Generate default values to ensure regime diversity
                mean_return = returns.iloc[-10:].mean() if len(returns) >= 10 else 0
                features['sma_trend_short'] = mean_return if not pd.isna(mean_return) else 0.0
                mean_return_20 = returns.iloc[-20:].mean() if len(returns) >= 20 else 0
                features['sma_trend_medium'] = mean_return_20 if not pd.isna(mean_return_20) else 0.0
                mean_return_50 = returns.iloc[-50:].mean() if len(returns) >= 50 else 0
                features['sma_trend_long'] = mean_return_50 if not pd.isna(mean_return_50) else 0.0
            
            # Momentum features - critical for regime differentiation
            if len(close) >= 10:
                # Price momentum over different periods
                momentum = (close.iloc[-1] - close.iloc[-10]) / close.iloc[-10]
                features['momentum'] = momentum if not pd.isna(momentum) else 0.0
                if len(close) >= 5:
                    momentum_5d = (close.iloc[-1] - close.iloc[-5]) / close.iloc[-5]
                    features['momentum_5d'] = momentum_5d if not pd.isna(momentum_5d) else features['momentum']
                else:
                    features['momentum_5d'] = features['momentum']
            else:
            ...
            if len(close) >= 14:
                ...
                rsi_val = rsi.iloc[-1] / 100.0  # Normalize to 0-1
                features['rsi'] = rsi_val if not pd.isna(rsi_val) else 0.5
                
                # MACD
                macd_line = ta.trend.MACD(close).macd()
                macd_signal = ta.trend.MACD(close).macd_signal()
                macd_div = (macd_line.iloc[-1] - macd_signal.iloc[-1]) / close.iloc[-1]
                features['macd_divergence'] = macd_div if not pd.isna(macd_div) else 0.0
            else:
                features['rsi'] = 0.5
            ...
            if volume.sum() > 0:
                ...
                if len(volume) >= 20 and volume_sma.iloc[-1] > 0:
                    vol_ratio = volume.iloc[-1] / volume_sma.iloc[-1]
                    features['volume_ratio'] = vol_ratio if not pd.isna(vol_ratio) else 1.2
                else:
            ...
            if len(close) >= 14:
                atr_val = atr.average_true_range().iloc[-1] / close.iloc[-1]
                features['atr_ratio'] = atr_val if not pd.isna(atr_val) else 0.02
            else:
                features['atr_ratio'] = 0.02
            
            # Higher timeframe alignment - enhanced to ensure diversity
            if len(close) >= 50:
                rolling_min_20 = close.rolling(20).min().iloc[-1]
                rolling_max_20 = close.rolling(20).max().iloc[-1]
                rolling_min_50 = close.rolling(50).min().iloc[-1]
                rolling_max_50 = close.rolling(50).max().iloc[-1]
                
                if rolling_max_20 > rolling_min_20:
                    pos_20d = (close.iloc[-1] - rolling_min_20) / (rolling_max_20 - rolling_min_20)
                    features['price_position_20d'] = pos_20d if not pd.isna(pos_20d) else 0.5
                else:
                    features['price_position_20d'] = 0.5
                    
                if rolling_max_50 > rolling_min_50:
                    pos_50d = (close.iloc[-1] - rolling_min_50) / (rolling_max_50 - rolling_min_50)
                    features['price_position'] = pos_50d if not pd.isna(pos_50d) else 0.5
                else:
                    features['price_position'] = 0.5
            else:
                # Use shorter periods for diversity
                min_period = min(20, len(close) - 1)
                if min_period > 5:
                    rolling_min = close.rolling(min_period).min().iloc[-1]
                    rolling_max = close.rolling(min_period).max().iloc[-1]
                    if rolling_max > rolling_min:
                        pos = (close.iloc[-1] - rolling_min) / (rolling_max - rolling_min)
                        features['price_position_20d'] = pos if not pd.isna(pos) else 0.5
                        features['price_position'] = features['price_position_20d']
                    else:
                        features['price_position_20d'] = 0.5
                        features['price_position'] = 0.5
                else:
            ...
            # Final NaN check - ensure no NaN values in any feature
            for key, value in features.items():
                if pd.isna(value):
                    # Use default values for any remaining NaN
    ...
    def detect_regime_ml(self, df: pd.DataFrame) -> Tuple[MarketRegime, float]:
        """ML-based regime detection with ensemble approach and robust fallback"""
        try:
            if not self.sklearn_available:
                self.logger.debug("Scikit-learn not available, using fallback regime detection")
            ...
            # Use clustering if model not trained
            if not self.model_trained and len(self.feature_history) >= self.min_training_features:
                self._train_initial_model()
            ...
            if self.model_trained:
                try:
                    ...
                    # Ensemble prediction
                    rf_pred = self.regime_classifier.predict_proba(scaled_features)[0]
                    gb_pred = self.ensemble_classifier.predict_proba(scaled_features)[0]
                    
                    # Combine predictions
                    ensemble_pred = (rf_pred + gb_pred) / 2
                    regime_idx = np.argmax(ensemble_pred)
                    confidence = ensemble_pred[regime_idx]
                    
                    # Map to all 9 regimes for comprehensive detection
                    regime_mapping = [
                    ...
                    detected_regime = regime_mapping[min(regime_idx, len(regime_mapping) - 1)]
                    
                except Exception as pred_error:
                    self.logger.warning(f"ML prediction failed: {pred_error}. Using rule-based fallback")
                    detected_regime, confidence = self._rule_based_regime_detection(features)
            
            else:
                # Use rule-based approach until model is trained
                self.logger.debug("ML model not trained, using rule-based detection")
                detected_regime, confidence = self._rule_based_regime_detection(features)
    ...
    def _train_initial_model(self):
        """Train initial regime classification model"""
        try:
            # WALK FORWARD OPTIMIZATION: Use configured minimum threshold
            if len(self.feature_history) < self.min_training_features:
                return
            
            # Prepare training data
            X = np.array([list(f.values()) for f in self.feature_history])
            
            # Check for and handle any remaining NaN values
            if np.any(np.isnan(X)):
                self.logger.warning("Found NaN values in training features, replacing with defaults")
                # Replace NaN with defaults
                X = np.nan_to_num(X, nan=0.0, posinf=0.0, neginf=0.0)
            
            # Create labels using rule-based classification for all 9 regimes
            y = []
            regime_counts = {}
            for features in self.feature_history:
                ...
                regime_labels = {
                    ...
                    MarketRegime.BREAKOUT_BULLISH: 5,
                    MarketRegime.BREAKOUT_BEARISH: 6,
                    MarketRegime.ACCUMULATION: 7,
                    MarketRegime.DISTRIBUTION: 8
                }
                label = regime_labels.get(regime, 2)
                y.append(label)
                
                # Count regime occurrences for debugging
                regime_name = regime.value if hasattr(regime, 'value') else str(regime)
                regime_counts[regime_name] = regime_counts.get(regime_name, 0) + 1
            
            y = np.array(y)
            
            # Debug: Log regime distribution
            self.logger.info(f"ML Training regime distribution: {regime_counts}")
            unique_classes = np.unique(y)
            self.logger.info(f"Unique class labels for ML training: {unique_classes}")
            self.logger.info(f"Total training samples: {len(y)}")
            self.logger.info(f"Feature matrix shape: {X.shape}")
            
            # Additional debugging - show which features are being used
            if len(self.feature_history) > 0:
                ...
                for i, (features, label) in enumerate(zip(self.feature_history[:3], y[:3])):
                    regime_name = [k for k, v in regime_labels.items() if v == label][0].value
                    self.logger.info(f"Sample {i}: regime={regime_name}, features={features}")
            ...
            try:
                # Check if we have enough diverse classes for ML training
                if len(unique_classes) < 2:
                    self.logger.warning(f"Insufficient class diversity for ML training: {len(unique_classes)} classes found. Falling back to rule-based detection.")
                    return
                
                # Log feature statistics for debugging
                self.logger.info(f"Training data shape: X={X_scaled.shape}, y={y.shape}")
                self.logger.info(f"Feature statistics: mean={np.mean(X_scaled, axis=0)[:5]}, std={np.std(X_scaled, axis=0)[:5]}")
            
                self.regime_classifier.fit(X_scaled, y)
                self.ensemble_classifier.fit(X_scaled, y)
                self.model_trained = True
                
                self.logger.info(f"ML regime models trained successfully with {len(unique_classes)} classes")
                
            except Exception as train_error:
                error_msg = str(train_error)
                if "1 class after sample_weight trimmed" in error_msg:
                    self.logger.warning(f"ML training failed due to insufficient class diversity: {error_msg}")
                    self.logger.info("This happens when rule-based detection assigns the same regime to all samples")
                    self.logger.info("Continuing with rule-based regime detection only")
                elif "min_samples_split" in error_msg or "min_samples_leaf" in error_msg:
                    self.logger.warning(f"ML training failed due to insufficient samples: {error_msg}")
                    # Try with reduced constraints
                    try:
                        from sklearn.ensemble import RandomForestClassifier, GradientBoostingClassifier
                        self.regime_classifier = RandomForestClassifier(n_estimators=50, min_samples_split=2, min_samples_leaf=1, random_state=42)
                        self.ensemble_classifier = GradientBoostingClassifier(n_estimators=50, min_samples_split=2, min_samples_leaf=1, random_state=42)
                        self.regime_classifier.fit(X_scaled, y)
                        self.ensemble_classifier.fit(X_scaled, y)
                        self.model_trained = True
                        
                        self.logger.info("ML models trained with reduced constraints")
                    except Exception as fallback_error:
                        self.logger.error(f"Fallback ML training also failed: {fallback_error}")
                else:
                    self.logger.error(f"Unexpected ML training error: {error_msg}")
                    
        except Exception as e:
            self.logger.error(f"Error in _train_initial_model: {e}")
            import traceback
            self.logger.error(traceback.format_exc())
    
    def _rule_based_regime_detection(self, features: dict) -> Tuple[MarketRegime, float]:
        """Enhanced rule-based regime detection for all 9 regimes with better diversity"""
        try:
            vol_ratio = features.get('volatility_ratio', 1.0)
            trend_short = features.get('sma_trend_short', 0.0)
            trend_medium = features.get('sma_trend_medium', 0.0)
            volatility = features.get('volatility_1d', 0.02)
            momentum = features.get('momentum', 0.0)
            volume_ratio = features.get('volume_ratio', 1.0)
            price_position = features.get('price_position_20d', 0.5)
            rsi = features.get('rsi', 0.5)
            
            # Create scoring system for better regime classification
            # ENHANCED: Make thresholds more sensitive to ensure regime diversity
            regime_scores = {}
            
            # High volatility regime - clear threshold separation
            vol_score = 0.0
            if volatility > 0.015 or vol_ratio > 1.5:  # Raised threshold to 1.5 for clear separation
                vol_score = min(1.0, (volatility - 0.015) * 50 + (vol_ratio - 1.0) * 3)
            regime_scores[MarketRegime.HIGH_VOLATILITY] = vol_score
            
            # Low volatility regime - mutually exclusive threshold
            low_vol_score = 0.0
            if volatility < 0.025 and vol_ratio < 1.0:  # Lowered to 1.0 to eliminate overlap
                low_vol_score = min(1.0, (0.025 - volatility) * 40 + (1.0 - vol_ratio) * 2)
            regime_scores[MarketRegime.LOW_VOLATILITY] = low_vol_score
            
            # Trending bull regime - lower threshold for detection
            bull_score = 0.0
            if trend_short > 0.001 and momentum > 0.005:  # Very low threshold
                bull_score = min(1.0, trend_short * 200 + momentum * 100)
                if trend_medium > 0:  # Bonus for medium trend alignment
                    bull_score += min(0.3, trend_medium * 100)
            regime_scores[MarketRegime.TRENDING_BULL] = bull_score
            
            # Trending bear regime - lower threshold for detection
            bear_score = 0.0
            if trend_short < -0.001 and momentum < -0.005:  # Very low threshold
                bear_score = min(1.0, abs(trend_short) * 200 + abs(momentum) * 100)
                if trend_medium < 0:  # Bonus for medium trend alignment
                    bear_score += min(0.3, abs(trend_medium) * 100)
            regime_scores[MarketRegime.TRENDING_BEAR] = bear_score
            
            # Breakout bullish - lower volume threshold
            breakout_bull_score = 0.0
            if volume_ratio > 1.15 and momentum > 0.008:  # Lower thresholds
                breakout_bull_score = min(1.0, (volume_ratio - 1.0) * 4 + momentum * 80)
                if price_position > 0.5:  # Bonus for higher price position
                    breakout_bull_score += min(0.2, (price_position - 0.5) * 2)
            regime_scores[MarketRegime.BREAKOUT_BULLISH] = breakout_bull_score
            
            # Breakout bearish - lower volume threshold
            breakout_bear_score = 0.0
            if volume_ratio > 1.15 and momentum < -0.008:  # Lower thresholds
                breakout_bear_score = min(1.0, (volume_ratio - 1.0) * 4 + abs(momentum) * 80)
                if price_position < 0.5:  # Bonus for lower price position
                    breakout_bear_score += min(0.2, (0.5 - price_position) * 2)
            regime_scores[MarketRegime.BREAKOUT_BEARISH] = breakout_bear_score
            
            # Accumulation regime - more sensitive detection
            accum_score = 0.0
            if abs(momentum) < 0.012 and volume_ratio > 1.05:  # Lower thresholds
                accum_score = min(1.0, (1.5 - abs(momentum) * 100) + (volume_ratio - 1.0) * 5)
                if rsi < 0.4:  # Bonus for oversold conditions
                    accum_score += min(0.3, (0.4 - rsi) * 3)
            regime_scores[MarketRegime.ACCUMULATION] = accum_score
            
            # Distribution regime - more sensitive detection
            dist_score = 0.0
            if abs(momentum) < 0.012 and price_position > 0.6:  # Lower thresholds
                dist_score = min(1.0, (1.5 - abs(momentum) * 100) + (price_position - 0.5) * 2)
                if rsi > 0.6:  # Bonus for overbought conditions
                    dist_score += min(0.3, (rsi - 0.6) * 3)
            regime_scores[MarketRegime.DISTRIBUTION] = dist_score
            
            # Ranging regime - default when nothing else scores high
            ranging_score = 0.0
            if abs(momentum) < 0.015 and abs(trend_short) < 0.008:  # Wider range
                ranging_score = min(1.0, (0.015 - abs(momentum)) * 50 + (0.008 - abs(trend_short)) * 100)
            regime_scores[MarketRegime.RANGING] = ranging_score
            
            # Add noise to prevent identical scores and ensure diversity
            import random
            for regime in regime_scores:
                regime_scores[regime] += random.uniform(-0.05, 0.05)
                regime_scores[regime] = max(0.0, regime_scores[regime])  # Keep non-negative
            
            # Find the regime with highest score
            best_regime = max(regime_scores.items(), key=lambda x: x[1])
            detected_regime = best_regime[0]
            
            # Ensure minimum confidence to avoid randomness
            confidence = min(0.95, max(0.4, best_regime[1]))
    ...
    def _fallback_regime_detection(self, df: pd.DataFrame) -> Tuple[MarketRegime, float]:
        ...
        try:
            ...
            price_trend = (close.iloc[-1] - sma_20) / sma_20
            
            if volatility > 0.03:
                return MarketRegime.HIGH_VOLATILITY, min(0.8, volatility * 20)
            elif price_trend > 0.02:
                return MarketRegime.TRENDING_BULL, min(0.8, abs(price_trend) * 30)
            elif price_trend < -0.02:
                return MarketRegime.TRENDING_BEAR, min(0.8, abs(price_trend) * 30)
            else:
    ...
    def get_regime_parameters(self, regime: MarketRegime, base_params: dict) -> dict:
        """Get regime-specific trading parameters"""
        try:
            adjusted_params = base_params.copy()
            
            if regime == MarketRegime.TRENDING_BULL:
                # More aggressive long positions, trend following
                adjusted_params['RSI_OVERSOLD'] = max(20, base_params.get('RSI_OVERSOLD', 30) - 10)
                adjusted_params['POSITION_SIZE_MULTIPLIER'] = 1.3
                adjusted_params['TAKE_PROFIT_MULTIPLIER'] = base_params.get('TAKE_PROFIT_MULTIPLIER', 2.5) * 1.2
                adjusted_params['min_confidence'] = max(0.1, base_params.get('min_confidence', 0.3) - 0.1)
                
            elif regime == MarketRegime.TRENDING_BEAR:
                # More defensive, quick exits
                adjusted_params['RSI_OVERBOUGHT'] = min(80, base_params.get('RSI_OVERBOUGHT', 70) + 10)
                adjusted_params['POSITION_SIZE_MULTIPLIER'] = 0.7
                adjusted_params['STOP_LOSS_MULTIPLIER'] = base_params.get('STOP_LOSS_MULTIPLIER', 1.5) * 0.8
                adjusted_params['min_confidence'] = min(0.8, base_params.get('min_confidence', 0.3) + 0.2)
                
            elif regime == MarketRegime.RANGING:
                # FIXED RANGING LOGIC: Mean reversion with adaptive take profits
                ...
                adjusted_params['POSITION_SIZE_MULTIPLIER'] = 1.0
                
                # CRITICAL FIX: Don't reduce take profits in ranging markets
                # Ranging markets still need reasonable profit targets to be profitable
                # Instead, use slightly tighter targets but not punitive reduction
                base_tp = base_params.get('TAKE_PROFIT_MULTIPLIER', 2.5)
                if base_tp >= 3.0:  # For high TP values, use moderate reduction
                    adjusted_params['TAKE_PROFIT_MULTIPLIER'] = base_tp * 0.9  # 10% reduction instead of 20%
                else:  # For reasonable TP values, keep them
                    adjusted_params['TAKE_PROFIT_MULTIPLIER'] = base_tp  # No reduction for reasonable values
                
            elif regime == MarketRegime.HIGH_VOLATILITY:
                # Risk reduction, higher confidence required
                adjusted_params['POSITION_SIZE_MULTIPLIER'] = 0.4
                adjusted_params['STOP_LOSS_MULTIPLIER'] = base_params.get('STOP_LOSS_MULTIPLIER', 1.5) * 0.7
                adjusted_params['min_confidence'] = min(0.8, base_params.get('min_confidence', 0.3) + 0.3)
                adjusted_params['strong_signal_threshold'] = min(0.9, base_params.get('strong_signal_threshold', 0.6) + 0.2)
                
            elif regime == MarketRegime.LOW_VOLATILITY:
                # Slightly more aggressive in low vol environments
                adjusted_params['POSITION_SIZE_MULTIPLIER'] = 1.1
                adjusted_params['min_confidence'] = max(0.1, base_params.get('min_confidence', 0.3) - 0.05)
                adjusted_params['TAKE_PROFIT_MULTIPLIER'] = base_params.get('TAKE_PROFIT_MULTIPLIER', 2.5) * 1.1
                
            elif regime == MarketRegime.BREAKOUT_BULLISH:
                # Aggressive momentum following
                adjusted_params['POSITION_SIZE_MULTIPLIER'] = 1.4
                adjusted_params['TAKE_PROFIT_MULTIPLIER'] = base_params.get('TAKE_PROFIT_MULTIPLIER', 2.5) * 1.3
                adjusted_params['TRAILING_STOP_MULTIPLIER'] = base_params.get('TRAILING_STOP_MULTIPLIER', 1.0) * 1.2
                adjusted_params['min_confidence'] = max(0.1, base_params.get('min_confidence', 0.3) - 0.15)
                
            elif regime == MarketRegime.BREAKOUT_BEARISH:
                # Very defensive, avoid new positions
                adjusted_params['POSITION_SIZE_MULTIPLIER'] = 0.3
                adjusted_params['min_confidence'] = min(0.9, base_params.get('min_confidence', 0.3) + 0.4)
                adjusted_params['STOP_LOSS_MULTIPLIER'] = base_params.get('STOP_LOSS_MULTIPLIER', 1.5) * 0.6
                
            elif regime == MarketRegime.ACCUMULATION:
                # Patient accumulation, lower confidence requirements
                adjusted_params['POSITION_SIZE_MULTIPLIER'] = 1.2
                adjusted_params['min_confidence'] = max(0.1, base_params.get('min_confidence', 0.3) - 0.1)
                adjusted_params['RSI_OVERSOLD'] = max(25, base_params.get('RSI_OVERSOLD', 30) - 5)
                adjusted_params['TAKE_PROFIT_MULTIPLIER'] = base_params.get('TAKE_PROFIT_MULTIPLIER', 2.5) * 1.15
                
            elif regime == MarketRegime.DISTRIBUTION:
                # Very cautious, avoid new longs, but don't kill take profits
                adjusted_params['POSITION_SIZE_MULTIPLIER'] = 0.5
                adjusted_params['min_confidence'] = min(0.8, base_params.get('min_confidence', 0.3) + 0.3)
                adjusted_params['RSI_OVERBOUGHT'] = min(75, base_params.get('RSI_OVERBOUGHT', 70) + 5)
                
                # FIXED: Don't reduce take profits - just make entries harder
                # The problem is getting into bad trades, not the profit targets
                # Keep stop losses tighter for capital preservation
                adjusted_params['STOP_LOSS_MULTIPLIER'] = base_params.get('STOP_LOSS_MULTIPLIER', 1.5) * 0.8
            
            return adjusted_params
            
        except Exception as e:
            self.logger.error(f"Error adjusting regime parameters: {e}")
            return base_params
