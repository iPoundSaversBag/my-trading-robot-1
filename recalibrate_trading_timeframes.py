#!/usr/bin/env python3
"""
Recalibrate production regime detector for 5m and 15m timeframes.
This script analyzes the actual trading timeframes to optimize thresholds.
"""
...
def create_production_config():
    """Create optimized production configuration for 5m and 15m timeframes."""
    logger.info("🔧 Creating production configuration for trading timeframes...")
    
    # Analyze both trading timeframes
    analysis_5m = analyze_timeframe_data("5m")
    analysis_15m = analyze_timeframe_data("15m")
    
    if not analysis_5m or not analysis_15m:
        logger.error("Failed to analyze timeframe data")
        return None
    
    # Create optimized configuration
    config = {
        "calibration_info": {
            "calibrated_for": ["5m", "15m"],
            "calibration_date": "2025-08-21",
            "accuracy_target": "98%",
            "data_periods": {
                "5m": analysis_5m['total_periods'],
                "15m": analysis_15m['total_periods']
            }
        },
        "timeframe_configs": {
            "5m": {
                ...
                "volume_threshold": analysis_15m['volume_thresholds']['high'],
            ...
            "15m": {
                "volatility_threshold": analysis_15m['volatility_thresholds']['medium'],
                "high_volatility_threshold": analysis_15m['volatility_thresholds']['high'],
                "extreme_volatility_threshold": analysis_15m['volatility_thresholds']['extreme'],
                "volume_threshold": analysis_15m['volume_thresholds']['high'],
                "momentum_threshold": analysis_15m['momentum_thresholds']['medium'],
                "strong_momentum_threshold": analysis_15m['momentum_thresholds']['strong'],
    ...
    try:
        if os.path.exists(master_path):
            ...
            master_cfg = {}
        # Attach legacy-style timeframe configs under a namespaced key for reference
        master_cfg.setdefault('legacy_timeframe_calibration', {})['multi_tf'] = config.get('timeframe_configs', {})
        # Update regime_detection thresholds if meaningful
        ...
        logger.info(f"✅ Calibration merged into {master_path}")
    except Exception as e:
        logger.error(f"Failed merging calibration: {e}")
    
    # Print key thresholds
    print("\n📊 CALIBRATED THRESHOLDS FOR TRADING TIMEFRAMES:")
    print("=" * 60)
    ...
    print(f"\n🕐 15-minute timeframe:")
    print(f"   • Volatility threshold: {config['timeframe_configs']['15m']['volatility_threshold']:.6f}")
    print(f"   • High volatility: {config['timeframe_configs']['15m']['high_volatility_threshold']:.6f}")
    print(f"   • Momentum threshold: {config['timeframe_configs']['15m']['momentum_threshold']:.6f}")
...
def main():
    ...
    print("Target timeframes: 5m (primary), 15m (secondary)")
    print("=" * 60)
    ...
    if config:
        ...
        print("✅ Production regime detector now optimized for 5m and 15m trading")
